...
for repo in Commons Frontend Coordinator StandardLibrary EclipsePlugin ZeroToHero FearlessTour; do
  echo "=== $repo"
  ...
  git -C "$repo" clean -x -d --force -e test/mainCoordinator/LocalResources.java -e test/mainController/LocalResources.java
done
